...
const SearchResultsPage = () => {
  ...
  // Use MountainView data for search results
  const searchResults = MountainView?.map(homestay => ({
    id: homestay.id,
    name: homestay.title || homestay.name || 'Unnamed Homestay',
    location: homestay.location || 'Location not specified',
    coordinates: { lat: 28.6139, lng: 77.2090 },
    price: homestay.discounted_price || homestay.currentPrice || 3000,
    originalPrice: (homestay.discounted_price || homestay.currentPrice || 3000) * 1.2,
    rating: homestay.ratings || 4.5,
    reviews: Math.floor(Math.random() * 200) + 50,
    images: homestay.images && homestay.images.length > 0 ? homestay.images : [homestay.imageUrl || "https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800"],
    ...
    bedrooms: Math.floor((homestay.homestay_capacity || 4) / 2),
    bathrooms: Math.floor((homestay.homestay_capacity || 4) / 3) + 1,
    description: homestay.uniqueness || 'Beautiful homestay with great amenities.',
